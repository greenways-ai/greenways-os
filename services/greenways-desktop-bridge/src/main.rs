...
use setup::{
    ...
    DesktopSetupResponse, SystemDesktopSetupInspector, DESKTOP_SETUP_PROTOCOL,
};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let setup_inspector = SystemDesktopSetupInspector::resolve()?;
    ...
    while let Some(line) = read_bounded_line(&mut input)? {
        ...
        if request_protocol(&line) == Some(DESKTOP_SETUP_PROTOCOL) {
