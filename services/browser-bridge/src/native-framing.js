...
function jsonBytes(value) {
  ...
  if (bytes.length > NATIVE_OUTPUT_LIMIT) {
    throw new Error("Native host output exceeds Chrome's 1 MB limit");
  }
  return bytes;
}
...
export class NativeMessageDecoder {
  ...
  push(chunk) {
    ...
    if (this.buffer.length > this.limit + 4) {
      throw new Error("Native message buffer exceeds the configured limit");
    }
    return output;
  }
}
