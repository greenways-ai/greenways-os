...
final class DesktopSetupSnapshot {
  ...
  factory DesktopSetupSnapshot.fromJson(Map<String, Object?> json) {
    ...
    final actions = _texts(json, 'permittedActions')
        .map(DesktopSetupOperation.fromWire)
        .toList(growable: false);
    final observedAtUnixMs = _integer(
      json,
      'observedAtUnixMs',
      positive: true,
    );
  ...
  bool get mandatoryReady {
    ...
    final byKind = {for (final component in components) component.kind: component};
    ...
    switch (state) {
      ...
      default:
        if (components.length != _componentOrder.length ||
            ...
            !_sameActions(
              permittedActions,
              const [DesktopSetupOperation.inspect],
            )) {
