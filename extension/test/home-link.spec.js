...
test("legacy Home Link action opens the signed browser-pairing surface", async ({ context, extensionId }) => {
  ...
  await expect(homeNode.getByRole("heading", {
    name: "Legacy Home Link remains available for migration.",
  })).toBeVisible();
  await homeNode.getByRole("button", { name: "Use legacy Home Link" }).click();

  const dialog = page.getByRole("dialog", { name: "Legacy Home Link" });
  await expect(dialog).toBeVisible();
  await expect(dialog.getByRole("heading", {
    name: "Pair only when preserving an existing Home Link.",
  })).toBeVisible();
  await expect(dialog.getByLabel("Legacy Home Node origin")).toHaveValue("http://127.0.0.1:58100");
  await expect(dialog.getByLabel("One-time pairing code")).toBeVisible();
  await expect(dialog).toContainText("Compatibility is not the new architecture.");
  await expect(dialog).toContainText("never evaluates remote JavaScript, Wasm, HAL");

  await dialog.getByRole("button", { name: "Close Legacy Home Link" }).click();
  await expect(dialog).toBeHidden();
  expect(errors).toEqual([]);
});

test("pairs and restores a signed legacy Home Link in the browser", async ({ context, extensionId }) => {
  ...
  try {
    ...
    await homeNode.getByRole("button", { name: "Use legacy Home Link" }).click();
    const dialog = page.getByRole("dialog", { name: "Legacy Home Link" });
    await dialog.getByLabel("Legacy Home Node origin").fill(app.origin);
    ...
    await dialog.getByRole("button", { name: "Pair legacy Home Link" }).click();

    await expect(dialog.getByRole("heading", {
      name: "Your original browser link remains active.",
    })).toBeVisible();
    ...
    await dialog.getByRole("button", { name: "Close Legacy Home Link" }).click();
    await expect(homeNode.getByRole("heading", {
      name: "This browser retains its original signed Home Link.",
    })).toBeVisible();
    ...
    await expect(restored.getByRole("button", { name: "Manage legacy link" })).toBeVisible();
    await restored.getByRole("button", { name: "Manage legacy link" }).click();
    const restoredDialog = page.getByRole("dialog", { name: "Legacy Home Link" });
    await expect(restoredDialog.getByRole("heading", {
      name: "Your original browser link remains active.",
    })).toBeVisible();
