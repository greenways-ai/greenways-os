...
test("reports installed, available, and update approval states", () => {
  const home = BUILTIN_APP_CATALOG.find(({ id }) => id === "greenways-home");
  const oldHestia = {
    ...BUILTIN_APP_CATALOG.find(({ id }) => id === "hestia-connector"),
    version: "0.2.0",
  };
  ...
  assert.equal(inventory.entries.find(({ id }) => id === "hestia-connector").status, "update-available");
});
